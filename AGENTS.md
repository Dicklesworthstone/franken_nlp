...
- `../asupersync` — the execution foundation: structured orchestration, cancellation, budgets, IO, bounded admission, and CPU-team lifecycle through `Cx::spawn_blocking` + whole-request/batch-epoch `Cx::scoped_cpu`. The historical plan-audited revision was `8eb48575889c81b65f7556db4b26d47a8bc03197`; `SUITE.lock`/Cargo select `362dc5b174427f66cfa76ab2bdd68cce1a95c6cc` for implementation. Contract claims must cite the selected revision or an explicit cross-revision comparison. At the selected pin, its Rayon entry is dev-only and does not enter the release graph. Leaf kernels perform math but never spawn. Its HTTP/TLS client (`tls-webpki-roots`) is the ONLY networked Rust/product code path (`fnlp pull`); no reqwest/hyper, ever. The explicit out-of-band `scripts/fetch_model.sh` / `scripts/fetch_model.ps1` provisioning tools may use system `curl` / PowerShell web APIs to fetch the pinned upstream source closure; they are not inference or library code. Later `362dc5…` shorthand in this file denotes that complete selected revision, never an independently floating prefix.
...
Nanbeige4.2-3B is a dense decoder-only Llama-family transformer with a loop. The committed truth pack promotes the 201-name census and absence of mHC/depth/LoopSplit/ngram tensor families to `[EVIDENCED]`; it also evidences two 22-layer passes, KV indexing as `layer + loop×22`, and **final RMSNorm after each pass, so loop 2 consumes loop-1 normalized output**, with 44 post-layer states plus two post-loop norm states. The pinned config/census dimensions are hidden 3072, 48 query / 8 KV heads, explicit head_dim 128, SwiGLU 10752, eps 1e-5, split-half RoPE θ=7e7, max positions 262144, vocab 166144, untied lm_head, and no bias/window. Evidence is clause-scoped: official-llama.cpp agreement, full RoPE/attention numerics, tokenizer byte-fallback/added-token/decode behavior, BOS/EOS boundaries, and template/parser task semantics remain `PARTIAL` or `OPEN` exactly as recorded in `docs/truth-pack/promotions.json` and the research register. Never promote the whole paragraph merely because one row is evidenced.
...
```bash
cargo fmt --check
cargo check --locked --all-targets --no-default-features --features production
cargo clippy --locked --all-targets --no-default-features --features production -- -D warnings
cargo test --locked --all-targets --no-default-features --features production
ubs --diff
```
...
### The locked `production` test leg (green-bar requirement after graph ratification)

The crate and `scripts/check.sh` now exist, but that does not create build
authority. The locked test leg becomes eligible only after the explicit
`production` graph and DSR recipe are ratified, then must exit 0 in a retained
controller-selected clean-SHA DSR checkpoint before a code-complete Bead can
close. The validation entrypoint must execute the core `production` graph,
separately inspect `production,metadata-store`, and run the isolated
`asupersync-census` lane; a missing, skipped, default-empty, or all-features
substitute blocks the verdict. Until the receipt binds the clean source SHA,
all named lanes, and literal `PASS|FAIL`, build authority is **BLOCKED**.

Build-surface note: both binaries compile from thin shims over `cli_main()` (doctrine #12); the locked `production` all-targets check MUST be free of the "present in multiple build targets" warning.
...
1. **The crate is broad.** `franken_nlp` is a single crate by design (plan §4.1), but no controller-authorized clean-SHA DSR receipt currently establishes its clean-build duration on a named host/graph. Record that duration at the first eligible checkpoint; do not repeat “takes minutes” as fact before then.
...
Repository Actions **MUST remain disabled**. The workflow file may remain inert
as historical context, but its status and logs are non-authoritative. Direct RCH runs are likewise
