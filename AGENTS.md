...
- `../asupersync` — structured-concurrency runtime, for **orchestration / cancellation / IO only** (not for intra-op math parallelism). Its HTTP/TLS client (`tls-webpki-roots`) is also the ONLY networked code path (`fnlp pull`); no reqwest/hyper, ever.
...
- The **complete** direct non-suite release allowlist is `clap`, `serde`/`serde_json`, and `sha2`, each only for the layer named in plan §3.4. Everything else must be `std` or a pinned FrankenSuite surface. **No `thiserror`, `anyhow`, `ctrlc`, `rayon`, `half`, `memmap2`, `uuid`, `num_cpus`, allocator crate, `tokenizers`, `tiktoken`, `minijinja`, or `llguidance`.** Tokenizer/template/grammar are ours, but tokenizer bytes are embedded only after LG-1.

Project source license is the exact text in `LICENSE`. It does **not** license Nanbeige weights/tokenizer or swiss_army_llama materials. Never collapse those provenance surfaces.
...
**Hard rule: no surface ships against an unresolved dependency.** Promote source observations only with immutable hash + line span + replay fixture. LG-1 blocks public assets, not local kernel research.
...
16. **Licensing fails closed.** No public weights, embedded tokenizer, copied preset, or “Apache cleared” wording until LG-1 evidence and owner review exist.
...
- **Model-gated e2e**: ordinary CI reports `SKIPPED_NO_MODEL`; release certification requires an armed pass with artifact digest and fallbacks pointed at `/nonexistent`.
