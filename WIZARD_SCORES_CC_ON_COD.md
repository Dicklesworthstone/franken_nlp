...
> **Post-review status (2026-07-30; refreshed 2026-07-31): historical cross-score, not specification.** This file evaluated pre-v3 snapshots; its hashes and plan-section/factual-audit statements intentionally describe those snapshots and are stale for the current files. The current master plan §10.6 is authoritative. At scoring time the plan carried a now-retired LG-1 publication gate; the owner's later ruling settled the pinned model-card Apache-2.0 declaration, and the current plan enforces a mechanical license/notice/modification bundle instead. It keeps `FeedForced` token-id exact (a unique byte run is insufficient) and rejects token healing as an exact/parity path; the preserved idea record now labels Copy-as-Decode withdrawn. This review's claims that legal/illegal margins are effectively free/always-on or that holdout coverage alone can validate schema inference were also rejected. Preserve the scores as adversarial provenance, not present project state.
