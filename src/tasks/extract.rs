...
//!
//! This is a structural JSON contract, not an accuracy or source-grounding
//! claim. Verbatim source products remain refused by the runtime compiler.
...
use crate::{
    ...
    grammar::{
        ...
        runtime::{JSON_RUNTIME_VERSION, JsonProgram},
    },
    native_engine::{
...
impl ExtractPlan {
    ...
    pub fn from_task_plan(
        task: &TaskPlan,
        schema_source: &str,
        mut options: JsonDecodeOptions,
        mut limits: CompileLimits,
        controls: &TemplateControlIds,
    ) -> Result<Self, ExtractError> {
        let ir = task.ir();
        ir.validate().map_err(|_| ExtractError::Contract("invalid TaskIR"))?;
        if task.task_spec_identity() != "extract-v1" || !matches!(ir.decode_strategy(), DecodeStrategy::ConstrainedJson) {
            return Err(ExtractError::Contract("extract-v1 requires constrained_json"));
        }
        check_postconditions(ir)?;
        ...
        match ir.grammar() {
            GrammarReference::JsonSchema { digest, compiler_version }
                if *digest == schema_digest && compiler_version == JSON_RUNTIME_VERSION => {}
        ...
        let program = JsonProgram::compile(schema_source, limits).map_err(ExtractError::Schema)?;
        let policy_digest = policy_digest(&options, limits, controls.ids())?;
        ...
        Ok(Self {
            program, prompt, options, controls: controls.ids().clone(), taskir_digest,
    ...
    pub fn bind_identity(&self, mut identity: ExecutionIdentity) -> Result<ExecutionIdentity, ExtractError> {
        ...
        identity.grammar_compiler_version = JSON_RUNTIME_VERSION.to_owned();
    ...
    pub fn verify_identity(&self, identity: &ExecutionIdentity) -> Result<(), ExtractError> {
        ...
        if identity.taskir_digest != self.taskir_digest || identity.prompt_digest != self.prompt_digest
            ...
            || identity.grammar_compiler_version != JSON_RUNTIME_VERSION || identity.sampler_version != EXTRACT_SAMPLER_VERSION
            || identity.tokenizer_digest != Sha256Digest::of_bytes(PINNED_TOKENIZER_MODEL_BYTES)
        {
    ...
    fn finalize(&self, output: JsonDecodeOutput) -> Result<ExtractResult, ExtractError> {
        ...
        self.program.validate_json(&output.json).map_err(|_| ExtractError::InvalidResult)?;
        let result = ExtractResult {
            schema_version: 1, task_spec_version: "extract-v1".to_owned(),
            score_space: ScoreSpace::NotComputed, grounding: ExtractionGrounding::NotRequested, output,
        };
...
fn check_profile(identity: &ExecutionIdentity) -> Result<(), ExtractError> {
    ...
    { return Err(ExtractError::Contract("requires extract-v1, eager bf16, thinking off, no tools")); }
    Ok(())
}
...
mod tests {
    ...
    fn options() -> JsonDecodeOptions { JsonDecodeOptions { max_new_tokens: 8, eos_token_id: 0, excluded_token_ids: BTreeSet::new() } }
    ...
    fn output(json: &str) -> JsonDecodeOutput {
