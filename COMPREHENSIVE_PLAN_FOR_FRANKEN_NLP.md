...
- **CPU-bound stages cross the native asupersync boundary once.** An admitted run enters through `Cx::spawn_blocking`; inside that blocking closure, the pinned `Cx::scoped_cpu(worker_cap, …)` surface creates the bounded lending fork/join team. Its children borrow run-local tensors safely, inherit the caller's `Cx`, checkpoint cancellation/budgets at tile or morsel boundaries, contain panics, and all join before the region returns. The scope spans an entire generation request or a bounded batch scheduling epoch—**never one layer, tile, or token**—because creating OS threads per micro-operation would destroy latency.
- **Intra-op parallelism is owned by asupersync's scoped CPU region; leaf kernels never spawn.** The coordinator participates as one logical worker and creates at most `physical_compute_width − 1` children (after reserving the measured orchestration/OS allowance). Exactly one latency forward occupies the admission domain; concurrent library callers wait through a bounded asupersync session/semaphore policy or join a compatible throughput batch. They never create independent oversubscribing scopes. Streaming uses bounded, two-phase channels integrated with the same cancellation/drain state machine.
...
- **Cancellation:** a cheaply cloned asupersync `Cx` (or a narrower checkpoint capability proven by the Phase-0 probe) is threaded into long loops; `checkpoint()` runs at named per-doc, prefill-morsel, and per-decode-step boundaries. Cancellation is cooperative and honored at the next boundary; per-doc budgets in batch mode prevent one pathological document from stalling the stream. `Cx` is **Clone, not Copy**, at the inspected pin.
