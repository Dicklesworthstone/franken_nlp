...
Status: **resolved for the code-first implementation wave; batch verification
pending.**  This is the independent design record for `franken_nlp-g6f`.  The
writer (`franken_nlp-rsk`) and reader (`franken_nlp-vk7`) must implement the
frozen v1 specification below, not infer a broader format from an early
implementation.  A change to a decision in this record is a plan-revision
event, not an implementation tweak.
...
The companion fixtures are normative successor inputs:

| Evidence artifact | SHA-256 | Purpose |
|---|---|---|
| `docs/specs/fnlpq-envelope-v1.md` | `0d4c3c672790c3025adddc03d08d4ffc5df7cdcebd118428b026cd0fe7806686` | Frozen byte layout, JSON rules, digest domains, caps, and dispatch policy. |
| `tests/fixtures/fnlpq/field_inventory.json` | `098bb4421b4e0ec35e5c148e3ecacb0e3a36d2b0be7f3d4455d622fb379c0d58` | Writer golden inventory for every prelude, directory, and header field. |
| `tests/fixtures/fnlpq/hostile_cases.json` | `c62a38d93b7d896718c79599d41801043fd841fa3860ab36730494df97e2a81f` | Reader corpus with 18 stable malformed/dispatch cases and required diagnostics. |
...
## Clause 1: header versus binary-directory authority

**Settled:** the prelude and binary section directory own all stored byte
...
## Clause 2: digest-domain taxonomy

**Settled:** the six normative names are `source_root_sha256`,
...
## Clause 3: duplicate keys, names, and non-finite values

**Settled:** duplicate JSON keys, tensor names, section names, singleton
...
## Clause 4: packing-set multiplicity and dispatch

**Settled:** a `PackingSet` may contain several named representations only
...
## Clause 5: malformed-file limits and parser order

**Settled:** prelude validation precedes every variable allocation.  The
...
## Successor obligations and closure evidence
...
This record resolves OQ-31's five design clauses for the code-first wave.
`franken_nlp-g6f` remains **in progress** until central batch verification can
confirm the artifact hashes, fixture parsing, and successor integration
evidence; it is not closed by this commit.
